...
- **nemo-guardrails-custom**: Standalone NeMo Guardrails server (config_id `phi3-judge`, image pinned by digest), backed by llm-d-inference-sim; used by the `[Full]`-tagged guardrails e2e tests. Optional, in `nemo-guardrails/`; deployed via `make deploy-nemo-guardrails-test-servers`
