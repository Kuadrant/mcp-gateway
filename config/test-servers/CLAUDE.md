...
- **llm-d-inference-sim**: OpenAI-compatible response simulator (not real inference) used as the judge-model backend for nemo-guardrails-custom
- **nemo-guardrails-custom**: Standalone NeMo Guardrails server (config_id `phi3-judge`), backed by llm-d-inference-sim; used by the `[Full]`-tagged guardrails e2e tests
