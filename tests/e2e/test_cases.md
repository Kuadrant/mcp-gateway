...
- When multiple MCPServerRegistration resources are created pointing to servers with prompts and different prefixes, a prompts/list request should return prompts from all registered servers, each prefixed with the corresponding server's prefix. For example, server1's "greet" prompt should appear as "s1_greet" and everything-server's prompts should appear as "everything_simple_prompt", "everything_complex_prompt", etc.
