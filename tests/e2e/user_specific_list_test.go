...
var _ = Describe("MCP Gateway User-Specific Tool Lists", func() {
	...
	It("[Happy,UserSpecificList] user sees their own tools merged with cached tools", func() {
		...
		userAClient, err := NewMCPGatewayLegacyClientWithHeaders(ctx, gatewayURL, map[string]string{
	...
	It("[UserSpecificList] different users get different tool lists", func() {
		...
		userBClient, err := NewMCPGatewayLegacyClientWithHeaders(ctx, gatewayURL, map[string]string{
	...
	It("[UserSpecificList] standard servers unaffected by userSpecificList", func() {
		...
		mcpClient, err := NewMCPGatewayLegacyClient(ctx, gatewayURL)
		Expect(err).NotTo(HaveOccurred())
		...
		noAuthClient, err := NewMCPGatewayLegacyClient(ctx, gatewayURL)
		Expect(err).NotTo(HaveOccurred())
	...
	It("[Security,UserSpecificList] internal headers not forwarded to upstream", func() {
		...
		userClient, err := NewMCPGatewayLegacyClientWithHeaders(ctx, gatewayURL, map[string]string{
			"Authorization":       "Bearer user-a-token",
	...
	It("[UserSpecificList] virtual server filter applies to user-specific tools", func() {
		...
		vsClient, err := NewMCPGatewayLegacyClientWithHeaders(ctx, gatewayURL, map[string]string{
			"Authorization":       "Bearer user-a-token",
