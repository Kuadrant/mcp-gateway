...
| `userSpecificList` | String (`Enabled` / `Disabled`) | No | When `Enabled`, the broker fetches tools from this server per-user using their session headers instead of caching the service account's tool list. Default: `Disabled` |
