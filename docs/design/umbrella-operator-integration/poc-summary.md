...
Trigger: kuadrant-operator startup. The mcp-gateway controller is deployed as a cluster singleton in the kuadrant-operator namespace alongside other component controllers (authorino-operator, limitador-operator, dns-operator). It runs unconditionally — no Kuadrant CR required.
...
`replaces` edge, generates an upgrade InstallPlan, and the new kuadrant-operator starts — the mcp-gateway controller is already deployed as a singleton on startup, and the existing MCPGatewayExtension continues to be reconciled without touching the broker-router.
...
## Alignment with RFC 0019 (updated 2026-07-29)

The RFC was significantly updated on 2026-07-29. Key changes that affect this POC summary:

- **Controller deployment trigger**: the mcp-gateway controller deploys on kuadrant-operator startup unconditionally as a cluster singleton, not triggered by MCPGatewayExtension or Kuadrant CR events. Conditional component deployment is deferred to a future RFC.
- **Runtime rendering**: all child operator resources (CRDs, ClusterRoles, Deployments) are applied at runtime by kuadrant-operator on startup via Helm chart rendering. They are not included in the OLM bundle.
- **No `static/` split**: the previously considered approach of extracting CRDs and ClusterRoles into installer-managed paths is explicitly rejected in the RFC due to OLM GVK conflicts during upgrades.
- **`escalate` verb restored**: required because child operator ClusterRoles grant permissions the kuadrant-operator itself does not hold.
