...
- `api/v1alpha1/zz_generated.deepcopy.go`, `config/crd/bases/mcp.kuadrant.io_mcpgatewayextensions.yaml` and `charts/mcp-gateway/crds/` — regenerated by `make generate-all`, listed so the task's scope includes its outputs
...
- [ ] Validation rejects non-positive values. There is deliberately no upper bound tied to the ext_proc `message_timeout`: that bounds each ext_proc message, not the request lifetime, so coupling them would cut healthy long-running tool calls.
