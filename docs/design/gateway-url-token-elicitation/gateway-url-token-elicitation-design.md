...
| `headerName` | string | Optional. The header name used to inject the token. Defaults to `Authorization`. Allows injection via custom headers (e.g., `X-API-Key`). |
| `headerValueFormat` | string | Optional. A format string for the header value, where `{token}` is replaced with the raw token from the form. Defaults to `Bearer {token}`. Examples: `Bearer {token}`, `token {token}`, `{token}` (raw, no prefix). |
...
type TokenURLElicitationConfig struct {
    ...
    HeaderValueFormat string `json:"headerValueFormat,omitempty"` // default: "Bearer {token}"
}
```
...
A traditional CSRF token is not needed. The broker's POST `/tokens` endpoint requires the `mcp-session-id` custom header carrying the gateway session JWT. Browsers cannot set custom headers on cross-origin requests without CORS preflight approval, so a malicious site cannot forge a POST with the correct header. This custom header requirement, combined with single-use elicitation IDs and short TTLs, provides equivalent protection to a CSRF token without additional state.
