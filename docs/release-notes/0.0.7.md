...
### Progressive tool discovery (`discover_tools` / `select_tools`)

When `--discovery-tools-enabled` is true (default), the broker exposes two meta-tools that let MCP clients browse lightweight server metadata then scope which federated tools appear in `tools/list`. Selection is per MCP session; `select_tools` with an empty list resets to the full visible tool set. `--discovery-tool-threshold` defaults to `0` (never hide by count); values greater than zero activate progressive hiding when the session-visible tool count exceeds the threshold. `MCPServerRegistration` gains optional `category` ([]string) and `hint` fields carried into broker config for catalog entries.
