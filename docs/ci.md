...
| Target | Specs | When |
|-|-|-|
| `pr` | ~32 specs (curated happy path) | every PR |
| `pr-extended` | ~64 specs (excludes Full/multi-gateway) | on demand |
| `full` | all specs | nightly, on demand |

## Make Targets

```bash
make test-e2e-pr                      # PR gate (~32 specs)
make test-e2e-pr-extended             # broader coverage (~64 specs)
...
Valid suite names are defined in `build/known-suites.txt`.
