...
The controller injects the corresponding `OAUTH_*` env vars into the broker-router deployment automatically. `resourceName`, `bearerMethodsSupported`, and `scopesSupported` are optional and have defaults (`"MCP Gateway"`, `["header"]`, and `["basic"]` respectively). `resource` defaults to `https://<publicHost>/mcp` if not set.
