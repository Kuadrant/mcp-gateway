...
is_known_suite() {
  # fixed-string match: label-derived names must never act as a regex
  grep -qwF -- "$1" "$KNOWN_SUITES_FILE"
}

# suites that need extra infrastructure (ci-auth-setup deploys keycloak + cert-manager)
INFRA_SUITES="auth-policy tls multi-gateway"

needs_infra() {
  local suite_name infra_name
  for suite_name in "${suites[@]}"; do
    for infra_name in $INFRA_SUITES; do
      [ "$suite_name" = "$infra_name" ] && return 0
    done
  done
  return 1
}
...
emit() {
  ...
  local infra=false
  if [ "$is_full" = "true" ] || needs_infra; then
    infra=true
  fi
