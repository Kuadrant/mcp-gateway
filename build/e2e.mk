...
# pr: curated happy-path set (~32 specs), runs on every PR
E2E_SUITE_PR = --label-filter="pr"
# pr-extended: current broader set (excludes Full and multi-gateway; the char
# class also catches combined tags like [Full,CACertBundle])
...
test-e2e-happy: test-e2e-deps ## Quick e2e test run for local development (pr suite, no setup)
	...
	$(GINKGO) -v --tags=e2e --procs=$(E2E_PROCS) --timeout=$(E2E_TIMEOUT) $(E2E_SUITE_PR) ./tests/e2e
...
.PHONY: test-e2e-pr
test-e2e-pr: test-e2e-deps ## Run PR-gate e2e tests (~32 specs, curated happy-path set)
	$(GINKGO) -v --tags=e2e --procs=$(E2E_PROCS) --timeout=$(E2E_TIMEOUT) --fail-fast $(E2E_SUITE_PR) ./tests/e2e

.PHONY: test-e2e-pr-extended
test-e2e-pr-extended: test-e2e-deps ## Run extended PR coverage (~64 specs, excludes Full/multi-gateway)
	$(GINKGO) -v --tags=e2e --procs=$(E2E_PROCS) --timeout=$(E2E_TIMEOUT) --fail-fast $(E2E_SUITE_PR_EXTENDED) ./tests/e2e
...
# SUITE is validated with make's own filter so an arbitrary value can never
# reach the shell via the label-filter expansion
.PHONY: test-e2e-suite
test-e2e-suite: test-e2e-deps ## Run a named e2e suite (SUITE=core|routing|sessions|...)
	@if [ -z "$(filter $(SUITE),$(E2E_KNOWN_SUITES))" ]; then \
		echo "usage: make test-e2e-suite SUITE=<name>"; \
		echo "known suites: $(E2E_KNOWN_SUITES)"; \
		exit 1; \
	fi
	$(GINKGO) -v --tags=e2e --procs=$(E2E_PROCS) --timeout=$(E2E_TIMEOUT) --fail-fast --label-filter="$(filter $(SUITE),$(E2E_KNOWN_SUITES))" ./tests/e2e
...
test-e2e-auth-ci: test-e2e-deps ## Run auth e2e tests only (requires ci-auth-setup)
	$(GINKGO) -v --tags=e2e --procs=$(E2E_PROCS) --timeout=$(E2E_TIMEOUT) --fail-fast --label-filter="auth-policy" ./tests/e2e
...
test-e2e-https: test-e2e-deps ## Run HTTPS-focused E2E tests (requires cert-manager + MCP_PAT)
	...
	$(GINKGO) -v --tags=e2e --procs=$(E2E_PROCS) --timeout=$(E2E_TIMEOUT) --fail-fast --label-filter="tls" ./tests/e2e
...
test-e2e-list-suites: ## List available e2e suites
	...
	@echo "  pr            curated happy-path (~32 specs)"
	@echo "  pr-extended   broader coverage (~64 specs)"
	@echo "  full          all specs"
