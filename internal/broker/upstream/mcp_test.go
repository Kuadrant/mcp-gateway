...
func TestBuildHTTPClient_GatewayCACertPEM_Success(t *testing.T) {
	caPEM, caKey, caCert := generateSelfSignedCA(t)
	serverCert := generateServerCert(t, caCert, caKey)
	...
	up := NewUpstreamMCP(&config.MCPServer{
		Name:   "gateway-ca-test",
		URL:    srv.URL + "/mcp",
		CACert: "", // empty server CACert
	}, string(caPEM)) // valid gateway CA
...
func TestBuildHTTPClient_GatewayCACertPEM_WrongCA(t *testing.T) {
	_, caKey, caCert := generateSelfSignedCA(t)
	serverCert := generateServerCert(t, caCert, caKey)
	...
	up := NewUpstreamMCP(&config.MCPServer{
		Name:   "gateway-ca-wrong-test",
		URL:    srv.URL + "/mcp",
		CACert: "",
	}, string(wrongCaPEM)) // wrong gateway CA
	...
	require.NoError(t, err)
	_, err = httpClient.Do(req) //nolint:bodyclose // expected to fail
	require.Error(t, err, "wrong gateway CA should not verify server cert")
}
