...
type entityRegistry[E any, S any] struct {
	...
	toServer func(E) S
	// getMetaFields extracts the AdditionalFields map from a server-side entity
...
// toServerItem converts an upstream entity to its gateway-facing server item.
// Replaces toolToServerTool / promptToServerPrompt.
	...
	for _, candidate := range toAdd {
		...
		for existingName, existingPtr := range gatewayItems {
			...
			if !ok {
				...
				r.logger.Error("unable to check conflict, id is missing",
			...
			if !ok {
				...
				r.logger.Error("unable to check conflict, id is not a string",
					"upstream mcp server", r.serverID, "type", reflect.TypeOf(existingID))
				continue
			}
...
func newToolRegistry(prefix, serverID string, gatewayServer ToolsAdderDeleter, listFromMCP func(context.Context) ([]mcp.Tool, error), logger *slog.Logger) *entityRegistry[mcp.Tool, server.ServerTool] {
	return newEntityRegistry[mcp.Tool, server.ServerTool](
		func(t mcp.Tool) string { return t.Name },
		func(st server.ServerTool) string { return st.Tool.Name },
		func(t mcp.Tool) server.ServerTool {
		...
		func(st server.ServerTool) map[string]any {
			if st.Tool.Meta == nil {
				return nil
			}
			return st.Tool.Meta.AdditionalFields
		},
		gatewayServer.AddTools,
		gatewayServer.DeleteTools,
		func() map[string]*server.ServerTool { return gatewayServer.ListTools() },
		listFromMCP,
		logger,
		serverID,
		"tool",
...
func newPromptRegistry(prefix, serverID string, promptsServer PromptsAdderDeleter, listFromMCP func(context.Context) ([]mcp.Prompt, error), logger *slog.Logger) *entityRegistry[mcp.Prompt, server.ServerPrompt] {
	return newEntityRegistry[mcp.Prompt, server.ServerPrompt](
		func(p mcp.Prompt) string { return p.Name },
		func(sp server.ServerPrompt) string { return sp.Prompt.Name },
		func(p mcp.Prompt) server.ServerPrompt {
			p.Name = prefixedName(prefix, p.Name)
			p.Meta = mcp.NewMetaFromMap(map[string]any{
				gatewayServerID: serverID,
			})
		...
		func(sp server.ServerPrompt) map[string]any {
			if sp.Prompt.Meta == nil {
				return nil
			}
			return sp.Prompt.Meta.AdditionalFields
		},
		func(sp ...server.ServerPrompt) {
			if promptsServer != nil {
				promptsServer.AddPrompts(sp...)
			}
		},
		func(names ...string) {
			if promptsServer != nil {
		...
		func() map[string]*server.ServerPrompt {
			if promptsServer == nil {
				return nil
			}
