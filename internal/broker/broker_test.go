...
func TestToolHints(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			hints, exists := b.ToolHints(tc.serverName, tc.toolName)
			if tc.shouldFail {
				require.False(t, exists, "expected no hints to be found")
				return
			}
			require.True(t, exists, "expected hints to be found")
			require.Equal(t, tc.readOnly, hints.ReadOnly, "readOnly mismatch: %#v", hints)
			require.Equal(t, tc.idempotent, hints.Idempotent, "idempotent mismatch: %#v", hints)
			require.Nil(t, hints.Destructive)
			require.Nil(t, hints.OpenWorld)
		})
	}
}
