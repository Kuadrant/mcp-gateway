...
func (m *mcpBrokerImpl) OnConfigChange(ctx context.Context, conf *config.MCPServersConfig) {
	...
	// when gateway CA cert changes, all managers must be recreated
	gatewayCACertChanged := m.gatewayCACertPEM != newGatewayCACert
	if gatewayCACertChanged {
		m.logger.InfoContext(ctx, "gateway CA certificate bundle changed, recreating all managers")
	...
	for _, mcpServer := range servers {
		...
		if ok {
			...
			if gatewayCACertChanged || mcpServer.ConfigChanged(man.Config()) {
