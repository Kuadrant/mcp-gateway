...
type MCPBroker interface {
	...
	// ToolHints returns the raw annotation hints for a served tool.
	// present for every served tool; individual hints are nil when the
	// upstream left them unspecified (mark3labs *bool semantics).
	ToolHints(serverID config.UpstreamMCPID, tool string) (upstream.ToolHints, bool)
...
func (m *mcpBrokerImpl) OnConfigChange(ctx context.Context, conf *config.MCPServersConfig) {
	...
	toStop := m.deregisterStaleManagers(ctx, servers)
...
func (m *mcpBrokerImpl) deregisterStaleManagers(ctx context.Context, servers []*config.MCPServer) []upstream.ActiveMCPServer {
	m.mcpLock.Lock()
	...
	for _, mcpServer := range servers {
		...
		if ok && mcpServer.ConfigChanged(man.Config()) {
...
func (m *mcpBrokerImpl) ToolHints(serverID config.UpstreamMCPID, tool string) (upstream.ToolHints, bool) {
