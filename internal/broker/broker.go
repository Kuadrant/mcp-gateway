...
func (m *mcpBrokerImpl) OnConfigChange(ctx context.Context, conf *config.MCPServersConfig) {
	...
	for _, mcpServer := range servers {
		...
		if _, ok := m.mcpServers[mcpServer.ID()]; !ok {
			...
			manager, err := upstream.NewUpstreamMCPManager(upstream.NewUpstreamMCP(mcpServer, conf.CACertBundle), m.listeningMCPServer, m.listeningMCPServer, m.logger.With("sub-component", "mcp-manager"), m.managerTickerInterval, m.invalidToolPolicy)
			if err != nil {
